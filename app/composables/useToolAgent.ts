...
export function useToolAgent() {
  ...
  async function startToolAgent(modelId: string, input: Record<string, unknown>) {
    ...
    const request = toolAgentRequest(modelId, input, t)
    ...
    if (!lab.canCreateAgent.value)
      throw new Error(t('Cannot create a new agent right now. Wait for the current turn to finish or check the agent limit.'))

    lab.createAgent()
    lab.draft.value = request
    const sent = await lab.sendMessage()
    if (!sent)
      throw new Error(lab.error.value || t('Could not start the agent. Your request is saved in its draft.'))
